...
/** Класс общих методов проверки значений в рамках ответов API */
    public class MethodsForCheckResponse {
        ...
        //Проверяем, что поле в теле ответа имеет нужное значение
        @Step("Check a value of a field from body response")
        public static void checkHashOfFieldFromBody(Response response, String field, HashMap<String, String> value){
            response.then().assertThat()
                .body(field, equalTo(value));
        }

//    //Проверяем, что поле в теле содержит нужное значение
//    @Step("Check a value of a field contains a value from body response")
//    public static void checkContainsValueOfFieldFromBody(Response response, String field, String value){
//        response.then().assertThat()
